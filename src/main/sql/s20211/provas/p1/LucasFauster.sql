...
CREATE FUNCTION resultado(p_pesquisa int, p_bairros varchar[], p_cidades varchar[]) RETURNS TABLE(pergunta int, histograma integer[]) AS $$
DECLARE
BEGIN
    RETURN QUERY 
        SELECT pesq.numero as pesquisa, ARRAY[perg.numero, array_ndims(array_agg(esc.resposta))] histograma FROM pesquisa pesq
            INNER JOIN pergunta perg ON perg.pesquisa = pesq.numero
            INNER JOIN resposta resp ON resp.pergunta = perg.numero
            INNER JOIN escolha esc ON esc.resposta = resp.numero
            INNER JOIN entrevista ent ON esc.entrevista = ent.numero
            INNER JOIN bairro bar ON bar.numero = ent.bairro
            INNER JOIN cidade cid ON cid.numero = bar.cidade
            WHERE pesq.numero = p_pesquisa 
                AND bar.nome = ANY(p_bairros) 
                AND cid.nome = ANY(p_cidades)
            GROUP BY pesq.numero, perg.numero;
END;
$$ LANGUAGE plpgsql;
